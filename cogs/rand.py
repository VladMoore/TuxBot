...
class randCog(commands.Cog):
    ...
    async def rand(self, ctx,*args):
        ...
        if len(args) < 2:
           embed=discord.Embed(title="Error: Missing Args!",description="**You are missing the following arguments in order**: \n    • **Length** of the array\n  • **Data Type**, the data type must be ‘uint8’ (returns integers between 0–255), ‘uint16’ (returns integers between 0–65535) or ~~hex16~~ (Currently not working).", color=discord.Color.red())
        ...
        elif args[1] not in ['uint16','uint8']:
            embed=discord.Embed(title="Error: Incorrect Values!",description=f"You inputted `{args[1]}` which is not an accepted **data type**, you must include either: **Uint16** or **Uint8**", color=discord.Color.red())
            await ctx.send(embed=embed)
        else:
